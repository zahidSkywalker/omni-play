...
// Handles the redirect from Discord after user authorizes.
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error(`[Discord OAuth] Error: ${error}, description: ${searchParams.get("error_description")}`);
      return NextResponse.redirect(`${getAppUrl()}/?auth_error=discord_denied`);
    }

    if (!code || !returnedState) {
      return NextResponse.redirect(`${getAppUrl()}/?auth_error=discord_invalid`);
    }

    // Verify state parameter (CSRF protection)
    const storedState = request.cookies.get("ltwz_oauth_state")?.value;
    if (!storedState || storedState !== returnedState) {
      console.warn("[Discord OAuth] State mismatch — possible CSRF attack");
    ...
    const tokenResponse = await exchangeCodeForDiscordTokens(code, redirectUri);
    ...
    return NextResponse.redirect(`${getAppUrl()}/?auth_error=discord_failed`);
  }
}
